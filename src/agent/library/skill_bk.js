...
export async function executeWorldEditCommand(bot, command) {
    /**
     * Executes a WorldEdit command and returns the server's feedback.
     * @param {MinecraftBot} bot - reference to the minecraft bot.
     * @param {string} command - The WorldEdit command to execute (e.g., "schem load schematic_file_name", "paste", "undo"). The schematic_file_name is usually `{schematicID}.schematic`.
     * @returns {Promise<string>} The feedback message from the server, or a timeout message.
     * @example
     * const feedback = await skills.executeWorldEditCommand(bot, "paste");
     * log(bot, feedback);
     **/
    if (!command) {
        const msg = "No WorldEdit command provided.";
        log(bot, msg);
        return msg;
    }

    const fullCommand = `/worldedit:/${command}`;
    log(bot, `Executing WorldEdit command: ${fullCommand}`);

    return new Promise((resolve) => {
        const timeout = setTimeout(() => {
            bot.removeListener('message', messageListener);
            const msg = "WorldEdit command timed out waiting for feedback.";
            log(bot, msg);
            resolve(msg);
        ...
        const messageListener = (message) => {
            ...
            // This is a heuristic to identify system messages. It might need adjustment.
            // It assumes player chat is formatted like <Player> text.
            if (!text.startsWith('<')) {
                // We assume the next system message is the feedback from WorldEdit.
                clearTimeout(timeout);
                bot.removeListener('message', messageListener);
                log(bot, `WorldEdit feedback: ${text}`);
                resolve(text);
            }
        };
...
/**
 * Converts a block's properties object into a Minecraft command-compatible string.
 * Example: { facing: 'north', half: 'bottom' } -> '[facing=north,half=bottom]'
 * @param {object} properties - The block's properties object.
 * @returns {string} - The formatted properties string or an empty string.
 **/
function propertiesToString(properties) {
    if (!properties || Object.keys(properties).length === 0) {
        return '';
    }
    const propsArray = Object.entries(properties).map(([key, value]) => `${key}=${value}`);
    return `[${propsArray.join(',')}]`;
}

/**
 * A helper function to compare if two schematic blocks are truly identical,
 * including their name and all of their properties.
 * @param {object} blockA - The first block.
 * @param {object} blockB - The second block.
 * @returns {boolean} - True if the blocks are identical.
 **/
function isSameBlock(blockA, blockB) {
    // If one block doesn't exist, they aren't the same.
    if (!blockA || !blockB) return false;

    // If their names are different, they aren't the same.
    if (blockA.name !== blockB.name) return false;

    // Get properties of both blocks.
    const propsA = blockA.getProperties();
    const propsB = blockB.getProperties();

    // A fast and reliable way to compare two simple key-value objects is to stringify them.
    // This correctly handles cases where properties might be null, undefined, or have different key orders.
    return JSON.stringify(propsA) === JSON.stringify(propsB);
}

/**
 * Builds a structure from a .schem file using optimized /fill commands with property-awareness.
 * This method is significantly faster and more accurate than using /setblock for each block.
 * NOTE: This method requires the bot to have operator permissions on the server.
 *
 * @param {MinecraftBot} bot - Reference to the mineflayer bot with OP privileges.
 * @param {string} schematicId - The ID/name of the .schem file to build.
 **/
export async function buildSchematic(bot, schematicId) {
    try {
        const schemFilePath = `/Volumes/datadisk/projects/minecraft-schematic-go/data/minecraft-schematics/schematics/${schematicId}.schematic`;
        console.log(`[Builder] Reading schematic file: ${schemFilePath}`);
        const schemBuffer = await fs.readFile(schemFilePath);
        const schematic = await Schematic.read(schemBuffer);

        const origin = bot.entity.position.floored();
        const { size, offset } = schematic;

        const skipBlockTypes = new Set(['air', 'minecraft:air', 'minecraft:cave_air', 'minecraft:void_air', 'minecraft:water']);

        // --- 1. PRE-PROCESSING STAGE ---
        const blocks = Array.from({ length: size.x }, () => 
                         Array.from({ length: size.y }, () => 
                           Array.from({ length: size.z }, () => null)
                         )
                       );

        console.log(`[Builder] Pre-processing schematic data...`);
        for (let y = 0; y < size.y; y++) {
            for (let z = 0; z < size.z; z++) {
                for (let x = 0; x < size.x; x++) {
                    const posInSchem = new Vec3(x, y, z);
                    const block = schematic.getBlock(posInSchem.add(offset));
                    if (block && !skipBlockTypes.has(block.name)) {
                        blocks[x][y][z] = block;
                    }
                }
            }
        ...
        // --- 2. GREEDY MESHING STAGE ---
        const visited = Array.from({ length: size.x }, () => 
                          Array.from({ length: size.y }, () => 
                            Array.from({ length: size.z }, () => false)
                          )
                        );

        const fillCommands = [];
        console.log(`[Builder] Generating property-aware optimized /fill commands...`);

        for (let y = 0; y < size.y; y++) {
            for (let z = 0; z < size.z; z++) {
                for (let x = 0; x < size.x; x++) {
                    if (!visited[x][y][z]) {
                        const startBlock = blocks[x][y][z];
                        if (!startBlock) {
                            visited[x][y][z] = true;
                            continue;
                        }
                        ...
                        // Expand along X
                        let x2 = x;
                        while (x2 + 1 < size.x && !visited[x2 + 1][y][z] && isSameBlock(blocks[x2 + 1][y][z], startBlock)) {
                            x2++;
                        }
                        ...
                        let z2 = z;
                        let canExpandZ = true;
                        while (z2 + 1 < size.z && canExpandZ) {
                            for (let i = x; i <= x2; i++) {
                                if (visited[i][y][z2 + 1] || !isSameBlock(blocks[i][y][z2 + 1], startBlock)) {
                                    canExpandZ = false;
                                    break;
                                }
                            }
                        ...
                        let y2 = y;
                        let canExpandY = true;
                        while (y2 + 1 < size.y && canExpandY) {
                            for (let k = z; k <= z2; k++) {
                                for (let i = x; i <= x2; i++) {
                                    if (visited[i][y2 + 1][k] || !isSameBlock(blocks[i][y2 + 1][k], startBlock)) {
                                        canExpandY = false;
                                        break;
                                    }
                                }
                        ...
                        // Mark all blocks in this new cuboid as visited
                        ...
                        const startPos = origin.plus(new Vec3(x, y, z).minus(offset));
                        const endPos = origin.plus(new Vec3(x2, y2, z2).minus(offset));
                        
                        // *** MODIFICATION HERE: Append properties string to the block name ***
                        const blockProperties = startBlock.getProperties()
                        if (blockProperties && blockProperties.waterlogged) {
                            delete blockProperties.waterlogged;
                        }
                        const propertiesString = propertiesToString();
                        const fullBlockName = `${startBlock.name}${propertiesString}`;
                        
                        const command = `/fill ${startPos.x} ${startPos.y} ${startPos.z} ${endPos.x} ${endPos.y} ${endPos.z} ${fullBlockName}`;
        ...
        // --- 3. EXECUTION STAGE ---
        console.log(`[Builder] Generated ${fillCommands.length} precise commands.`);
        console.log(`[Builder] Starting construction...`);
        for (let i = 0; i < fillCommands.length; i++) {
            const command = fillCommands[i];
            try {
                // console.log(command); // Uncomment for debugging
                await bot.chat(command);
                await wait(50); 
            } catch (e) {
                console.error(`[Builder] Failed to execute command: ${command}. Error: ${e.message}`);
            }
        }

        console.log("[Builder] Schematic construction complete!");
    } catch (err) {
        console.error("[Builder] A critical error occurred:", err);
    }
}
